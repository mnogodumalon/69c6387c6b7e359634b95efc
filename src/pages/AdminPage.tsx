...
const HAUPTPROMPT_FIELDS = [
  { key: 'prompt_id', label: 'ID', type: 'string/text' },
  { key: 'name', label: 'Name', type: 'string/text' },
  { key: 'zweck_kontext', label: 'Zweck / Kontext', type: 'string/textarea' },
  { key: 'prompt_text', label: 'Prompt-Text', type: 'string/textarea' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'aktiv', label: 'Aktiv' }, { key: 'archiviert', label: 'Archiviert' }, { key: 'entwurf', label: 'Entwurf' }] },
  { key: 'erstellt_am', label: 'Erstellt am', type: 'date/date' },
  { key: 'zuletzt_geaendert', label: 'Zuletzt geändert', type: 'date/date' },
  { key: 'vollstaendiger_prompt', label: 'Vollständiger Prompt (zusammengeführt)', type: 'string/textarea' },
  { key: 'prompt_email_action', label: 'Prompt per E-Mail versenden', type: 'lookup/radio', options: [{ key: 'send_email', label: 'E-Mail mit vollständigem Prompt versenden' }] },
];
const NACHBESSERUNG_FIELDS = [
  { key: 'haupt_prompt_link', label: 'Verknüpfter Haupt-Prompt', type: 'applookup/select', targetEntity: 'haupt_prompt', targetAppId: 'HAUPT_PROMPT', displayField: 'prompt_id' },
  { key: 'versionsnummer', label: 'Versionsnummer', type: 'string/text' },
  { key: 'nachbesserungs_text', label: 'Nachbesserungs-Text', type: 'string/textarea' },
  { key: 'aenderungsnotiz', label: 'Änderungsnotiz', type: 'string/textarea' },
  { key: 'erstellt_am_nb', label: 'Erstellt am', type: 'date/date' },
];

const ENTITY_TABS = [
  { key: 'haupt_prompt', label: 'Haupt-Prompt', pascal: 'HauptPrompt' },
  { key: 'nachbesserung', label: 'Nachbesserung', pascal: 'Nachbesserung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('haupt_prompt');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'haupt_prompt': new Set(),
    'nachbesserung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'haupt_prompt': return (data as any).hauptPrompt as HauptPrompt[] ?? [];
      case 'nachbesserung': return (data as any).nachbesserung as Nachbesserung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'nachbesserung':
        lists.haupt_promptList = (data as any).hauptPrompt ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'nachbesserung' && fieldKey === 'haupt_prompt_link') {
      const match = (lists.haupt_promptList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.prompt_id ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'haupt_prompt': return HAUPTPROMPT_FIELDS;
      case 'nachbesserung': return NACHBESSERUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'haupt_prompt': return {
        create: (fields: any) => LivingAppsService.createHauptPromptEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateHauptPromptEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteHauptPromptEntry(id),
      };
      case 'nachbesserung': return {
        create: (fields: any) => LivingAppsService.createNachbesserungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateNachbesserungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteNachbesserungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'haupt_prompt' || dialogState?.entity === 'haupt_prompt') && (
        <HauptPromptDialog
          open={createEntity === 'haupt_prompt' || dialogState?.entity === 'haupt_prompt'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'haupt_prompt' ? handleUpdate : (fields: any) => handleCreate('haupt_prompt', fields)}
          defaultValues={dialogState?.entity === 'haupt_prompt' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['HauptPrompt']}
          enablePhotoLocation={AI_PHOTO_LOCATION['HauptPrompt']}
        />
      )}
      {(createEntity === 'nachbesserung' || dialogState?.entity === 'nachbesserung') && (
        <NachbesserungDialog
          open={createEntity === 'nachbesserung' || dialogState?.entity === 'nachbesserung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'nachbesserung' ? handleUpdate : (fields: any) => handleCreate('nachbesserung', fields)}
          defaultValues={dialogState?.entity === 'nachbesserung' ? dialogState.record?.fields : undefined}
          haupt_promptList={(data as any).hauptPrompt ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Nachbesserung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Nachbesserung']}
        />
      )}
      {viewState?.entity === 'haupt_prompt' && (
        <HauptPromptViewDialog
          open={viewState?.entity === 'haupt_prompt'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'haupt_prompt', record: r }); }}
        />
      )}
      {viewState?.entity === 'nachbesserung' && (
        <NachbesserungViewDialog
          open={viewState?.entity === 'nachbesserung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'nachbesserung', record: r }); }}
          haupt_promptList={(data as any).hauptPrompt ?? []}
        />
      )}
